...
```{r}
daftarurl <- c('https://money.kompas.com/search/2018-07-0','https://money.kompas.com/search/2018-08-0','https://money.kompas.com/search/2018-09-0')
fetchurl <- function(daftarurl) {
  final_listurl <- data.frame(links = NULL, urls = NULL)
  for (j in 1:length(daftarurl)) {
    listurl <- data.frame(links = NULL, urls = NULL)
    for (i in 1:31) {
    current_url <- paste(daftarurl[j],toString(i),sep="")
    main.page <- read_html(current_url)
    #link URLs
    urls <- main.page %>% 
              html_nodes(".terkini__post") %>% # get the CSS nodes
              html_attr()
    links <- main.page %>% # feed `main.page` to the next step
              html_nodes(".terkini__title") %>% # get the CSS nodes
              html_text()
...
```{r}
daftarpages <- c(ceiling(1),ceiling(1),ceiling(1)) #Jumlah pages
daftarurl <-c(
  'https://www.detik.com/search/searchall?query=ekonomi%20bisnis&siteid=29&sortby=time&fromdatex=01/01/2018&todatex=31/12/2018&page=',
  'https://www.detik.com/search/searchall?query=ekonomi%20bisnis&siteid=3&sortby=time&fromdatex=01/01/2018&todatex=31/12/2018&page=',
  'https://www.detik.com/search/searchall?query=ekonomi%20bisnis&siteid=5&sortby=time&fromdatex=01/01/2018&todatex=31/12/2018&page=')
...
```{r}
beritaebis <-fetchberita(listurls)
```

```{R}
write.csv(beritaebis,'E:/Tugas Akhir/Fetch Berita/dataebis.csv')
beritadetik <- read.csv("E:/Tugas Akhir/Fetch Berita/dataebis.csv")
```

nambah data
```{R}
write.csv(beritaebis,'E:/Tugas Akhir/Fetch Berita/dataebisnew.csv')
beritadetik <- read.csv("E:/Tugas Akhir/Fetch Berita/dataebisnew.csv")
```
...
x <- data.frame(topik=rep("ekonomi bisnis", nrow(beritadetik)))
...
nambah data
```{r}
beritaebis <- read.csv("E:/Tugas Akhir/Fetch Berita/berita ekonomi bisnis.csv")
#beritaebis <- beritaebis[,-c(1:2)]
beritaebis <- rbind(beritaebis,beritadetik)
write.csv(beritaebis,'E:/Tugas Akhir/Data Edit/berita ekonomi bisnis.csv')
beritaebis <- read.csv("E:/Tugas Akhir/Data Edit/berita ekonomi bisnis.csv")
```

```{R}
write.csv(beritadetik,'E:/Tugas Akhir/Fetch Berita/berita ekonomi bisnis 2018.csv')
write.csv(beritadetik1,'E:/Tugas Akhir/Fetch Berita/berita ekonomi bisnis.csv')
beritaebis <- read.csv("E:/Tugas Akhir/Fetch Berita/berita ekonomi bisnis.csv")
```

```{R}
beritaebis <- beritaebis[,-c(1)]
```

```{r}
beritadetik <- read.csv("E:/Tugas Akhir/Fetch Berita/berita ekonomi bisnis 2018.csv")
...
write.csv(beritadetik1,'E:/Tugas Akhir/Fetch Berita/berita ebis tri I.csv')
write.csv(beritadetik2,'E:/Tugas Akhir/Fetch Berita/berita ebis tri II.csv')
write.csv(beritadetik3,'E:/Tugas Akhir/Fetch Berita/berita ebis tri III.csv')
write.csv(beritadetik4,'E:/Tugas Akhir/Fetch Berita/berita ebis tri IV.csv')
```
